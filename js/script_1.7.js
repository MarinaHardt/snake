...
let fruitImages; // Früchte sollen zufällig erscheinen
...
function snakeSpeed() {
  ...
  if (millis() - time > speed) {
    ...
    // Position 1 des Schwanzes wird definiert als Kopf der Schlange
    tail[0].x = head.x;
    tail[0].y = head.y;
    ...
    if (fruit.x === head.x && fruit.y === head.y) {
      // Schwanz wird erstmal außerhalb der Spielfelds gezeichnet
      tail.push({ x: -1, y: -1 });
      moreFruit();
...
// neue Frucht erscheinen, nachdem Schlange sie gegessen hat
function moreFruit() {
  do {
